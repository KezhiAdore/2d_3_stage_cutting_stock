...
class PatternGenerator:
    ...
    def item_require_num(self,length,width):
        ...
        raise ValueError("can not find length={},width={} in item list".format(length,width))
    
    def reduce_item_require(self,length,width,reduce_num=1):
        if length in self._item_require_num.keys():
            if width in self._item_require_num[length].keys():
                if self._item_require_num[length][width]>=0:
                    self._item_require_num[length][width]=max(self._item_require_num[length][width]-reduce_num,0)
                    return
                else:
                    raise ValueError(self._item_require_num[length][width],"should larger than 0")
        
        if width in self._item_require_num.keys():
            if length in self._item_require_num[width].keys():
                if self._item_require_num[width][length]>=0:
                    self._item_require_num[width][length]=max(self._item_require_num[width][length]-reduce_num,0)
                    return
                else:
                    raise ValueError(self._item_require_num[width][length],"should larger than 0")
    ...
    def generate_strips(self):
        ...
        for x in self._q:
            ...
            for shape in self._item_shape:
                ...
                e_max=min(int(x/item_length),self.item_require_num(item_length,item_width))
                strips[x].append(Strip(x,item_length,item_width,e_max,item_value))
        return strips
    ...
    def generate_segment_x(self,x):
        ...
        for y in np.arange(self._interval,self._W+self._interval,self._interval):
            ...
            if dp_F[y-self._interval]>=max_F:
                ...
                dp_F[y]=max_F
                dp_segment[y]=copy.deepcopy(dp_segment[y-math.ceil(item_width)])
                dp_segment[y].append(Strip(x,item_length,item_width,max_e,max_strip.v))
                for length in self._item_require_num.keys():
                    for width in self._item_require_num[length].keys():
                        dp_n[length][width][y]=dp_n[length][width][y-math.ceil(item_width)]
    ...
    def export_segment_figures(self):
        for i,segment in enumerate(self._segments):
            ...
            segment.plot(ax)
            plt.plot()
            figure_path=os.path.join(segment_figures_dir,"{}_{}.png".format(i,segment.x))
    ...
    def segments_item_amount(self,item_length,item_width):
        ...
        item_amount=sum(
                [
                    seg.item_amount(item_length,item_width)+seg.item_amount(item_width,item_length) 
    ...
    def export_segments(self,export_path):
        pass
